...
const generateJoke = async (vibe: Vibe): Promise<Joke> => {
  // Always create a fresh instance to capture the latest process.env.API_KEY
  const apiKey = process.env.API_KEY;
  if (!apiKey) {
    throw new Error("API_KEY_NOT_FOUND: The neural engine requires an authentication key.");
  }
  ...
  try {
    ...
    // Graceful error handling for missing keys or instability
    const msg = error.message || "Unknown anomaly";
    if (msg.includes("API key") || msg.includes("API_KEY")) {
      throw new Error("NEURAL_LINK_FAILED: Identity key invalid or missing.");
    }
    throw new Error("HUMOR_MATRIX_UNSTABLE: Please re-synchronize.");
  }
};
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 md:p-12 relative overflow-y-auto">
      ...
      <header className="text-center mb-12 animate-in fade-in zoom-in duration-1000">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400 text-[9px] font-bold tracking-[0.4em] uppercase mb-8 select-none">
        ...
        <p className="text-slate-400 text-lg md:text-xl font-light tracking-wide max-w-2xl mx-auto opacity-80">
          Precision-engineered humor for biological units.
        </p>
      </header>
      ...
      <main className="w-full flex flex-col items-center gap-10 max-w-4xl z-10">
        ...
        {loading ? (
          <div className="w-full max-w-2xl min-h-[350px] flex flex-col items-center justify-center p-12 glass rounded-[3.5rem] border-violet-500/20 shadow-2xl">
            <div className="w-20 h-20 border-2 border-violet-500/20 border-t-violet-500 rounded-full animate-spin mb-8"></div>
            <p className="font-mono text-violet-400 text-xs tracking-[0.5em] uppercase animate-pulse">{steps[loadingStep]}</p>
          </div>
        ) : joke ? (
          <div className="w-full max-w-2xl p-12 md:p-20 glass rounded-[4rem] border-white/10 shadow-2xl relative transition-all duration-700 hover:border-violet-500/30 group">
            <div className="space-y-16">
              ...
              <div className={`transition-all duration-1000 transform ${reveal ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 blur-md'}`}>
                ...
                <p className="text-3xl md:text-5xl font-display font-black bg-gradient-to-br from-white to-violet-300 bg-clip-text text-transparent italic leading-snug text-glow">
                  {joke.punchline}
                </p>
              </div>
            </div>
          ...
          <div className="w-full max-w-2xl min-h-[350px] flex flex-col items-center justify-center p-12 text-center glass rounded-[3.5rem] border-white/10 opacity-30 select-none">
            <div className="text-7xl mb-8 animate-bounce">⚡</div>
            <h2 className="text-2xl font-display font-black text-white uppercase tracking-tighter">System Idle</h2>
          </div>
        )}

        {error && (
          <div className="px-8 py-4 bg-red-500/10 border border-red-500/20 rounded-full text-red-400 text-[10px] font-mono tracking-widest uppercase flex items-center gap-3 animate-bounce">
        ...
        <div className="flex flex-col sm:flex-row gap-6 w-full max-w-md px-4">
          <button
            onClick={fetchJoke}
            disabled={loading}
            className="flex-1 group relative px-10 py-6 bg-white text-black font-black rounded-full hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl disabled:opacity-50"
          >
            <div className="flex items-center justify-center gap-4 relative z-10 tracking-[0.3em] uppercase text-[10px]">
              {loading ? (
                <div className="w-5 h-5 border-2 border-black border-t-transparent rounded-full animate-spin"></div>
              ) : (
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="group-hover:rotate-180 transition-transform duration-700">
                  <path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path>
                  <path d="M21 3v5h-5"></path>
                </svg>
              )}
              {joke ? 'Initialize Next' : 'Execute System'}
            </div>
          </button>
          
          {joke && (
            <button onClick={copy} className="px-8 py-6 glass text-white rounded-full hover:bg-white/10 transition-all active:scale-90 border-white/10 shadow-xl flex items-center justify-center" title="Copy to clipboard">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"></rect><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"></path></svg>
            </button>
          )}
        </div>
      </main>
      ...
      <footer className="mt-24 mb-12 opacity-30 text-[9px] font-mono tracking-[0.6em] text-white uppercase text-center select-none">
        Neural Matrix v3.5.2 • System Operational
      </footer>
    </div>
  );
};
