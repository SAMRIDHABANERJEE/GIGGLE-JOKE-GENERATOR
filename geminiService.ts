...
// Helper to create a fresh AI instance with the latest key
...
/**
 * Fix: Added missing export generateJokeVisual
 * Generates an image representing the joke using Gemini 2.5 Flash Image.
 */
export const generateJokeVisual = async (joke: Joke): Promise<string> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        { text: `Generate a funny, artistic, high-quality visual representation of this joke: Setup: ${joke.setup} Punchline: ${joke.punchline}` }
      ]
    },
    config: {
      imageConfig: {
  ...
  throw new Error("No visual generated by the neural engine.");
};
...
export const generateProImage = async (prompt: string, config: ImageConfig): Promise<string> => {
  ...
  if (!part || !part.inlineData) throw new Error("Image part missing");
  return `data:image/png;base64,${part.inlineData.data}`;
};
...
export const editImage = async (base64: string, prompt: string): Promise<string> => {
  ...
  const part = response.candidates[0].content.parts.find(p => p.inlineData);
  if (!part || !part.inlineData) throw new Error("Edited image part missing");
  return `data:image/png;base64,${part.inlineData.data}`;
};
...
export const generateVeoVideo = async (prompt: string, imageBase64?: string): Promise<string> => {
  ...
  while (!operation.done) {
    // Fix: Updated timeout to 10000ms as per guidelines
...
export const neuralChat = async (
  ...
  if (options.media) {
    parts.unshift({ inlineData: { data: options.media.data, mimeType: options.media.type } });
  }
